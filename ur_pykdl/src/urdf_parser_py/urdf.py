...
xmlr.reflect(
    Cylinder,
    params=[xmlr.Attribute('radius', float),
...
xmlr.reflect(
    Collision, params=[origin_element,
...
xmlr.reflect(Texture, params=[xmlr.Attribute('filename', str, required=False)])
...
xmlr.reflect(
    Inertia, params=[xmlr.Attribute(key, float) for key in Inertia.KEYS])
...
xmlr.reflect(
    JointCalibration,
    params=[xmlr.Attribute('rising', float),
...
class Joint(xmlr.Object):
    TYPES = [
        'unknown', 'revolute', 'continuous', 'prismatic', 'floating', 'planar',
        'fixed'
    ]
...
xmlr.reflect(
    ...
    params=[
        name_attribute,
        xmlr.Attribute('type', str), origin_element,
